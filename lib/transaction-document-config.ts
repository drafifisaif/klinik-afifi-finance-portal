...
export const SUPPLIER_PAYMENT_DOCUMENTS_BUCKET = "finance-documents";
...
export function documentUploadLabel(entityName: TransactionDocumentEntityName) {
  if (entityName === "supplier_purchases" || entityName === "supplier_purchase_entries") return "Upload Supplier Invoice";
  if (entityName === "supplier_payment_entries") return "Upload Supplier Payment Proof";
...
export function documentBucketForEntity(entityName: TransactionDocumentEntityName) {
  if (entityName === "supplier_purchases" || entityName === "supplier_purchase_entries") return SUPPLIER_INVOICES_BUCKET;
  if (entityName === "supplier_payment_entries") return SUPPLIER_PAYMENT_DOCUMENTS_BUCKET;
